...
struct HomeView: View {
    ...
    var body: some View {
        ...
        .task(id: active) {
            // Спрятанная вкладка прогревается ОДИН раз и затихает. Раньше она опрашивала
            // сервер раз в минуту, и это фоновое присваивание данных пересобирало вьюху на
            // ГЛАВНОМ потоке — а именно там покадрово едет сворачивание клавиатуры в чате,
            // вот жест и спотыкался (Слава поймал: без сети гладко, с сетью — лаги). Живой
            // опрос теперь только у открытой вкладки; свежесть на переключении даёт разовый
            // прогрев + мгновенная загрузка при активации (.task перезапустится). Прогрев —
            // только если данных ещё нет, чтобы уход с вкладки не тянул лишний запрос.
            guard active else {
                if home == nil {
